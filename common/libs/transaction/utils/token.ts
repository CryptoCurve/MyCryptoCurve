...
const enoughTokensViaTx = (t: ITransaction | Tx, tokenBalance: TokenValue | null) => {
  ...
  if (!t.data || t.data.length === 0) {
    ...
    const { _value } = ERC20.transfer.decodeInput(sdk.utils.eth.bufferToHex(t.data));
...
const encodeTransfer = (to: Address, value: TokenValue) =>
  sdk.utils.eth.toBuffer(
    ERC20.transfer.encodeInput({ _to: sdk.utils.eth.bufferToHex(to), _value: value })
  );
